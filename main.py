...
def download_file(url, destination):
    """Надежное поблочное скачивание тяжелых файлов для защиты от Protobuf parsing failed"""
    print(f"📥 Начинаю скачивание: {destination}...")
    with requests.get(url, stream=True) as r:
        ...
        with open(destination, 'wb') as f:
            for chunk in r.iter_content(chunk_size=8192):
                if chunk:
                    f.write(chunk)
    print(f"✅ Файл {destination} успешно сохранен!")

# Скачиваем веса модели и голоса надежным методом
if not os.path.exists("kokoro-v0.19.onnx"):
    download_file(MODEL_URL, "kokoro-v0.19.onnx")
...
async def generate_podcast(payload: dict):
    ...
    # ИИ-команда на естественный диалог двух ведущих на РУССКОМ языке
    system_prompt = (
        "[style: conversational, radio-show. language: ru. speakers: am_adam (male), af_bella (female). "
        "Read and act this Russian text as a live, emotional dialogue between two hosts. Add natural breaks.]\n\n"
        f"{clean_text}"
    )
